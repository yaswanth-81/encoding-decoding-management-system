...
// Step 1: Fetch email based on Faculty ID
if (isset($_POST['fetch_email'])) {
    ...
    if (empty($facultyid)) {
        ...
        $stmt = $conn->prepare("SELECT email FROM faculty WHERE facultyid = :facultyid");
...
if (isset($_POST['reset_password']) && $_SESSION['otp_verified']) {
    ...
    if ($new_password === $confirm_password) {
        ...
        $stmt = $conn->prepare("UPDATE faculty SET password = :password WHERE facultyid = :facultyid");
        $stmt->bindParam(':password', $hashed_password);
        $stmt->bindParam(':facultyid', $_SESSION['facultyid']);
        $stmt->execute();

        session_destroy(); // Clear session
        header("Location: facultylogin.php?msg=Password updated successfully.");
        exit();
    } else {
